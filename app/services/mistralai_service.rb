class MistralAIService
  def initialize
    ...
    api_key = ENV['MISTRAL_API_KEY'] || Rails.application.credentials.mistral_api_key
    raise "Mistral API key not configured. Set MISTRAL_API_KEY environment variable or add to Rails credentials" if api_key.blank?
  ...
  def ocr(image_file, content_type)
    ...
    # Rails.logger.debug ocr.client
    # result = ocr.process()
    # Rails.logger.debug "Mistral OCR result: "
    Rails.logger.debug "#{response.pages[0].markdown}"
